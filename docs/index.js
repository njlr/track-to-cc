...
var _trackParser = _interopRequireDefault(__webpack_require__(/*! ./track-parser.js */ "./src/track-parser.js"));
...
var convert = function convert() {
  var parsedData = input.value.split(/[\s]+/).map(function (x) {
    ...
    try {
      return x + '\n' + new _trackParser.default(x).dump();
    } catch (error) {
      return 'INVALID';
    }
  });
...
//
// May-04-2018      Code clean-up to make compatible with ES6 and modules
//
//  ** NEW USAGE **
//
...
function _default(strParse) {
  ...
  this.hasTrack2 = false; /////////////////////////// end member fields /////////////////

  var sTrackData = this.input_trackdata_str; //--- Get the track data
  ...
  if (strParse != '') {
    ...
    var nHasTrack2 = strParse.indexOf("="); //--- Set boolean values based off of character presence

    var bHasTrack1 = false;
    var bHasTrack2 = false;
    ...
    var bTrack1_2 = false;
    var bTrack1 = false;
    var bTrack2 = false; //--- Determine tracks present
    ...
    if (bTrack1_2) {
      ...
      var _arrayStrSwipe = new Array(4);

      _arrayStrSwipe = strCutUpSwipe.split("^");

      var _sAccountNumber, _sName, _sShipToName, _sMonth, _sYear;

      if (_arrayStrSwipe.length > 2) {
        this.account = stripAlpha(_arrayStrSwipe[0].substring(1, _arrayStrSwipe[0].length));
        this.account_name = _arrayStrSwipe[1];
        this.expMonth = _arrayStrSwipe[2].substring(2, 4);
        this.expYear = '20' + _arrayStrSwipe[2].substring(0, 2); //--- Different card swipe readers include or exclude the % in the front of the track data - when it's there, there are
    ...
    if (bTrack1) {
      ...
      if (arrayStrSwipe.length > 2) {
        ...
        this.expYear = sYear = '20' + arrayStrSwipe[2].substring(0, 2); //--- Different card swipe readers include or exclude the % in
        ...
        sTrackData = sTrackData + this.track2; //--- parse name field into first/last names
    ...
    if (bTrack2) {
      ...
      var _sYear2 = strParse.substr(nSeperator + 1, 2);

      var _sMonth2 = strParse.substr(nSeperator + 3, 2); // console.log(sCardNumber + ' -- ' + sMonth + '/' + sYear);


      this.account = sAccountNumber = stripAlpha(sCardNumber);
      this.expMonth = _sMonth2 = _sMonth2;
      this.expYear = _sYear2 = '20' + _sYear2; //--- Different card swipe readers include or exclude the % in the front of the track data - when it's there,
